...
for size in ["1.5B", "0.5B"]:
    ...
    print(f"\n{'='*85}")
    print(f"  {size} — fp, fn SEPARATE, quadratic max (n={n}) — BEST VAL ACC")
    print(f"{'='*85}")
    ...
    for ax_idx, rv in enumerate(rollout_plot_vals):
        ...
        ax.set_title(f"r = {rv}")
        plt.colorbar(im, ax=ax, shrink=0.85, label="Best Val Accuracy")

    plt.tight_layout()
    plt.savefig(f"fig_v4_{size}_heatmap.png", bbox_inches="tight", dpi=150)
    plt.close()
    print(f"Saved fig_v4_{size}_heatmap.png")
    ...
    fig.suptitle(f"{size}:  {eq_str}\n(Adj-R² = {best[5]:.3f},  RMSE = {best[6]:.4f})",
                 fontsize=13, y=1.04)
    plt.tight_layout()
    plt.savefig(f"fig_v4_{size}_slices.png", bbox_inches="tight", dpi=150)
    plt.close()
    print(f"Saved fig_v4_{size}_slices.png")
    ...
    ax.set_xlabel("Actual Best Validation Accuracy")
    ax.set_ylabel("Predicted Best Validation Accuracy")
    ax.set_title(f"{size}:  {eq_str}\nAdj-R² = {best[5]:.3f},  RMSE = {best[6]:.4f}", fontsize=11)
    ax.set_xlim(lims); ax.set_ylim(lims)
    ax.set_aspect("equal")
    plt.tight_layout()
    plt.savefig(f"fig_v4_{size}_pred_vs_actual.png", bbox_inches="tight", dpi=150)
    plt.close()
    print(f"Saved fig_v4_{size}_pred_vs_actual.png")
