...
git checkout origin/backup/analysis-sweeps-infra-2026-07-11 -- \
  sweeps/cais_v4q3_remainder.txt sweeps/cais_v4q15_remainder.txt PUSH_CAIS_QWEN_REDO.md
```
| sweep | model | cells |
|---|---|---|
...
(0.5B forgone for now.) Done bar allows a 2-step logging slack (`_step >= 232`); in practice nothing sat in
that band so it's equivalent to strict 234.

- **Model staging:** Qwen2.5-3B is staged on CAIS. **Confirm `MODELS/Qwen2.5-1.5B` exists**
  (`hf download Qwen/Qwen2.5-1.5B` if not).
