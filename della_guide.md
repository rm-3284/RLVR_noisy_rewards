...
**Contacts:** 
For any questions regarding the setup or usage of the Della cluster, please contact [Taiming Lu](mailto:tl0463@princeton.edu) via email or Slack.

## Overview
This guide is for using the Della cluster in Zhuang's group at Princeton University.
...
If you have not been approved for access to Della yet, but your research requires access, please contact Zhuang.
...
_If you have created SSH keys in the previous step, add:_
```bash
...
After you have configured SSH (see [Config SSH](#config-ssh) section), you can use VSCode or other editors with Remote-SSH extension to develop directly on the cluster.
...
1. Click the Remote Explorer icon in the left sidebar (or the remote connection button <img src="./images/remote_connection.png" alt="remote connection icon" width="20" height="16" style="display: inline; vertical-align: middle;"> in the bottom-left corner)
...
<details>
<summary>Priority system details</summary>
...
The higher the priority, the earlier the job is in the queue.
...
Account fairshares are separate (e.g., della vs pli), and all users in the account affect each other.
...
**Important:** On Della, not fully using your requested resources can also affect your fairshare. This includes CPU cores, RAM, GPU utilization, and GPU VRAM. A useful command is `jobstats $JOB_ID`. This command should be run often to monitor your resource usage for a specific job.
...
In addition to priority impact, Della posts some hard constraints on QOS.
...
In general, waiting time, job usage, and past usage together decide if you can get a job running.
...
![Storage Diagram](./images/storage.png)
...
- **`/tigerdata/zhuangl/vision-mix`**
  - 15 TB shared among the group.
  - It has a longer read/write time and cannot be accessed through a compute node.
...
We found that Della has different usage throughout the week and sometimes the usage is high and sometimes it is low. Please monitor the available GPUs and use them if there are any available. You can check GPU availability using the `shownodes` command, through the MyDella web portal, or using the `useful_scripts/gpu_availability.sh` script.

</details>

<details>
<summary><strong>The --constraint=40G GPUs give me different types of configurations</strong></summary>
...
Neuronic cluster guide: [here](../neuronic/)
...
**Contacts:**
For any questions regarding the setup or usage of the Della cluster, please contact [Taiming Lu](https://taiminglu.com) via [email](mailto:tl0463@princeton.edu) or Slack.
